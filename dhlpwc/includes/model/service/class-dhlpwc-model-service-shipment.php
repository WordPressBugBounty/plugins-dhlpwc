...
class DHLPWC_Model_Service_Shipment extends DHLPWC_Model_Core_Singleton_Abstract
{
    ...
    public function bulk($order_ids, $bulk_size, $bulk_service_options = array())
    {
        ...
        foreach ($order_ids as $order_id) {
            ...
            // Default option settings
            $default_ssn = $option_service->default_ssn($order_id, $preselected_options, $to_business);
            if ($default_ssn) {
                $option_service->add_key_to_stack(DHLPWC_Model_Meta_Order_Option_Preference::OPTION_SSN, $preselected_options);
            }
                ...
                switch ($preselected_option) {
                    ...
                    case (DHLPWC_Model_Meta_Order_Option_Preference::OPTION_SSN):
                        ...
                        if ($default_hide_sender) {
                            $hide_sender_address = json_encode( $settings_service->get_hide_sender_address()->to_array());
                            $option_data[DHLPWC_Model_Meta_Order_Option_Preference::OPTION_SSN] = $hide_sender_address;
                        }
                        break;
                }
            }
