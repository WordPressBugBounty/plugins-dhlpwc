...
class DHLPWC_Model_Service_Order_Meta_Option extends DHLPWC_Model_Core_Singleton_Abstract
{
    ...
    public function default_ssn($order_id, $options, $to_business)
    {
        $service = DHLPWC_Model_Service_Access_Control::instance();
        $send_ssn_checked = $service->check(DHLPWC_Model_Service_Access_Control::ACCESS_DEFAULT_SSN);
        ...
        // Disable automatic checking of SSN if there are no parceltypes for it
        if (!array_key_exists(DHLPWC_Model_Meta_Order_Option_Preference::OPTION_SSN, $allowed_shipping_options)
            || in_array(DHLPWC_Model_Meta_Order_Option_Preference::OPTION_SSN, $exclusions)) {
            return false;
        }
    ...
    protected function get_exclusions($allowed_shipping_options, $options)
    {
        ...
        foreach ($options as $option) {
            if (array_key_exists($option, $allowed_shipping_options)) {
                ...
                if ($option === DHLPWC_Model_Meta_Order_Option_Preference::OPTION_LQ) {
                    if (!in_array(DHLPWC_Model_Meta_Order_Option_Preference::OPTION_PS, $exclusions)) {
                        $exclusions[] = DHLPWC_Model_Meta_Order_Option_Preference::OPTION_PS;
                    }
                }
                if ($option === DHLPWC_Model_Meta_Order_Option_Preference::OPTION_PS) {
                    if (!in_array(DHLPWC_Model_Meta_Order_Option_Preference::OPTION_LQ, $exclusions)) {
                        $exclusions[] = DHLPWC_Model_Meta_Order_Option_Preference::OPTION_LQ;
                    }
                }
            }
        }
