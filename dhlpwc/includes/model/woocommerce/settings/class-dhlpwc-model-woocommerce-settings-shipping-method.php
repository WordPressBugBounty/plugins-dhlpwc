...
class DHLPWC_Model_WooCommerce_Settings_Shipping_Method extends WC_Shipping_Method
{
    ...
    public function __construct($instance_id = 0)
    {
        ...
        $this->id = 'dhlpwc';
        $this->method_title = __('DHL eCommerce (Benelux) for WooCommerce', 'dhlpwc');
    ...
    public function get_admin_options_html() {
        $view = new DHLPWC_Template('admin.settings-container');
        return $view->render(array(
            'admin_options_html' => parent::get_admin_options_html(),
        ), false);
    }
    ...
    public function init_form_fields()
    {
        ...
        switch ($country_code['country']) {
            case 'NL':
                $api_settings_manual_url = 'https://www.dhlecommerce.nl/sites/default/files/content/PDF/Handleiding_WooCommerce_koppeling_NL.pdf';
                break;
            default:
                $api_settings_manual_url = 'https://www.dhlecommerce.nl/sites/default/files/content/PDF/Manual_WooCommerce_plug-in_EN.pdf';
        }
        ...
        $this->form_fields = array_merge(
            ...
            array(
                'default_hide_sender_address_service'                   => array(
                    'title'       => __('Default enable hide sender address service', 'dhlpwc'),
                    'type'        => 'checkbox',
                    'label'       => __('Enable', 'dhlpwc'),
                    'description' => __("When creating a label, enable the hide sender address service by default.", 'dhlpwc'),
                    'default'     => 'no',
                ),
            ),
    ...
    public function calculate_shipping($package = array())
    {
        ...
        foreach ($presets as $data) {
            $preset = new DHLPWC_Model_Meta_Shipping_Preset($data);
            if ($this->get_option('enable_option_' . $preset->setting_id) !== 'yes') {
                continue;
            }

            if ($preset->setting_id === 'fallback') {
            ...
            if ($check_allowed_options !== true) {
                continue;
            }

            $title = $this->get_preset_title($preset);
            $cost = $this->calculate_cost($package, $preset->setting_id);
            $cost = $this->get_tax_adjusted_cost($cost);
        ...
        if (!$added_rate && $fallbackPreset !== false) {
            $title = $this->get_preset_title($fallbackPreset);
            $cost = $this->calculate_cost($package, $fallbackPreset->setting_id);
            $cost = $this->get_tax_adjusted_cost($cost);

            $meta_data = array();
            if ($sort_position = $this->get_option('sort_position_' . $fallbackPreset->setting_id)) {
                $meta_data = array('sort_position' => $sort_position);
            }

            if (!$this->disable_condition($fallbackPreset->setting_id, $package)) {
                $this->add_rate(array(
                    'id'        => 'dhlpwc-' . $fallbackPreset->frontend_id,
                    'label'     => $title,
                    'cost'      => $cost,
                    'meta_data' => $meta_data,
                ));
    ...
    protected function get_preset_title($preset)
    {
        ...
        if (!empty($alternate_text)) {
            if (!empty($domain)) {
                return __($alternate_text, $domain);
            } else {
                return $alternate_text;
            }
        }
        return __($preset->title, 'dhlpwc');
    }

    protected function get_tax_adjusted_cost($cost)
    {
